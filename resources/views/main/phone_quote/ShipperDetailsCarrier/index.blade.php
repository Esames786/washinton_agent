...
@section('content')
    ...
    <div class="page-header">
        ...
        <div class="text-secondary text-center text-uppercase w-100">
            <h1 class="my-4"><b>Carrier Approaching</b></h1>
        </div>
    </div>
    <!--End Page header-->
    <!-- Row -->
    <div class="row">
        <div class="col-12">
            ...
            <div class="card">
                @php
                    ...
                    if (!empty($query)) {
                        $ptype = $query['penal_type'];
                          ...
                          $datas = \App\ShipperDetailsCarrier::select('states', 'type', \DB::raw('COUNT(CASE WHEN user_id = 0 THEN 1 ELSE NULL END) as total_with_user_id_0'), \DB::raw('COUNT(*) as total'), \DB::raw('COUNT(CASE WHEN user_id = 0 THEN 1 ELSE NULL END) as remaining'))->where('states', '!=', '-')
                          ->groupBy('states','type')
                ...
                @endphp
                @if (in_array('157', $phoneaccess) || Auth::user()->role == 1 || Auth::user()->role == 9 || in_array('156', $phoneaccess))
                    @if (Auth::user()->role == 1 || Auth::user()->role == 9 || in_array('156', $phoneaccess))
                        <div class="card-header">
                            <div class="container-fluid">
                                <div class="col-lg-12 p-0">
                                        ...
                                        ?>
                                    <form method="post" action="{{ route('store.autos.approach_new_carrier') }}">
                                        @csrf
                                        <div class="row" style="border:groove;padding: 10px">
                                            <div class="col-lg-2">
                                            ...
                                            <div class="col-lg-3">
                                                <label style="float: left">Dispatchers</label>
...
{{--                                                <option value="">Select Type</option>--}}
                                        ...
                                        <div class="col-lg-4">
                                            ...
                                            <select id="template" name="template" class="form-control template" >
                                                @foreach($template as $vl)
                                                    <option value="{{ $vl->id }}">
                                                        {{ $vl->name }}
                                                    </option>
                                                @endforeach
                                        ...
                                        <div class="col-lg-4">
                                            <label style="float: left">Limit</label>
                                            <input type="text" min="0" max="10000" placeholder="limit" class="form-control" name="limit_data" id="limit_data">
                                        </div>
                                        <div class="col-lg-3 mt-auto">
                                            <button class="btn btn-primary" type="button" id="send_template">
                                                Send Email Template to selected filter
                                            </button>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-lg-2">
                                            <a class="btn btn-primary" href="{{ route('all.autos.autos_approach_new_carrier') }}">
                ...
                <div class="card-body">
                    <div id="table_data">
                        ...
                        @include('main.phone_quote.ShipperDetailsCarrier.table')
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            <div class="modal-body history-content">
                                <form action="{{ route('Admin.autos_approach_new.Store_carrier') }}" method="POST"
...
@section('extraScript')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            function fetchData(page,send_template = null,template = null,limit_data = null) {
                ...
                $.ajax({
                    url: '{{ route('autos.approach_new.search_carrier') }}' + '?page=' + page,
                    type: 'GET',
                    data: {
                        state: stateValue,
                        search: searchValue,
            ...
            $('#send_template').on('click', function(event) {
                var template = $('#template').val();
                var limit_data = $('#limit_data').val();
                fetchData(1,1,template,limit_data);
                alert('Email Will be Released after 5 minutes');
            });
    ...
    <script>
        $(document).ready(function () {
            ...
            function updateSuggestions(inputField, suggestionsList) {
                ...
                $.ajax({
                    url: '{{ route('autos_approach_new.validation.check_carrier') }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        field_name: 'name',
    ...
    <script>
        $(document).ready(function () {
            $('#withHistory').change(function () {
                ...
                $.ajax({
                    url: '{{ route('autos.approach_new.search_carrier') }}',
                    type: 'GET',
                    data: {
