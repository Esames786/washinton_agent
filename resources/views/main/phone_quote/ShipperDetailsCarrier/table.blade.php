                ...
                <td>
                    ...
                    <script>
                        function openWhatsApp(phone, approachId) {
                            ...
                            $.ajax({
                                url: '{{ route('autosapproachnew.phone.count_carrier') }}',
                                type: 'GET',
                                data: {
                    ...
                    @else
                        <form method="POST" action="{{ route('autosApproachNew.save.email_carrier') }}"
                              data-compid="{{ $val->id }}">
                            @csrf
                            ...
                            <script>
                                $(document).ready(function() {
                                    ...
                                    $('.addEmailForm[data-compid="{{ $val->id }}"] .submitBtn').on('click', function() {
                                        ...
                                        // Ajax request
                                        $.ajax({
                                            url: '{{ route('autosApproachNew.save.email_carrier') }}',
                                            type: 'POST',
                                            data: formData,
                ...
                <div class="modal-body">
                    <div class="col-md-12">
                        <div class="form-group">
                            <form id="addHistoryForm" action="{{ route('autosapproachnew.store.call.history_carrier') }}" method="POST"
...
<script>
    ...
    function call(num, approachId) {
        ...
        }else if(check_call == 158){
            num1 = num1.replace(/[^\d+]/g, '');

            // Use Zoom Phone deep link (requires Zoom Phone app)
            window.location.href = 'tel:' + num1;
    ...
    $(".add-history").click(function() {
        ...
        $.ajax({
            url: '{{ route('autosapproachnew.call.history_carrier') }}',
            type: 'GET',
            data: {
