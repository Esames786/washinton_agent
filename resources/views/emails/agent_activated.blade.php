      ...
      <tr>
        <td style="padding:36px 40px;">
          ...
          <p style="margin:0 0 20px;color:#555;font-size:14px;line-height:1.7;">
            Great news! Your <strong>{{ $brand['name'] ?? 'Hello Transport' }} Agent Portal</strong> account has been reviewed and activated by our admin team. You can now log in and start working.
          </p>
            ...
            <tr>
              <td style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:20px 24px;">
                ...
                <p style="margin:0 0 16px;color:#555;font-size:13px;">
                  @php $loginUrl = $brand['login_url'] ?? 'https://hellotransport.com/loginn'; @endphp
                  <a href="{{ $loginUrl }}" style="color:#062e39;word-break:break-all;">
                    {{ $loginUrl }}
                  </a>
                </p>
          ...
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td align="center">
                <a href="{{ $brand['login_url'] ?? 'https://hellotransport.com/loginn' }}"
      ...
      <tr>
        <td style="background:#f8f9fa;padding:20px 40px;text-align:center;border-top:1px solid #e9ecef;">
          <p style="margin:0 0 6px;color:#6c757d;font-size:12px;">
            If you did not register for a {{ $brand['name'] ?? 'Hello Transport' }} account, please ignore this email.
          </p>
          <p style="margin:0;color:#adb5bd;font-size:12px;">
            © {{ date('Y') }} {{ $brand['footer'] ?? 'Hello Transport. All Rights Reserved.' }}<br>
