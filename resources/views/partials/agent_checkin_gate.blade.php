    ...
    try {
        if (Auth::user()->role != 1) {
            ...
            if ($__hrEmp) {
                ...
                // Shift ENDED: the agent checked out, so the CRM must lock again. Previously the
                // gate only looked at check_in, so once someone checked in the portal stayed open
                // for the rest of the day even after checking out. HR's own rule (RULE 3 in
                // EmployeeAttendanceController::getAttendanceForCard) disables re-check-in for the
                // rest of the shift once check_out is set, so this can never strand an agent.
                $__shiftEnded = (bool) ($__att && $__att->check_in && $__att->check_out);
            }
        }
    } catch (\Throwable $e) {}
...
@if($__needCheckin || ($__shiftEnded ?? false))
<style>
...
<div id="agentCheckinGate" style="position:fixed;inset:0;z-index:99985;background:rgba(15,23,42,.6);display:flex;align-items:center;justify-content:center;padding:16px;">
    <div style="background:#fff;border-radius:14px;max-width:480px;width:100%;box-shadow:0 24px 70px rgba(0,0,0,.45);text-align:center;padding:32px 28px;font-family:'Segoe UI',Arial,sans-serif;">
        @if($__shiftEnded ?? false)
            <div style="font-size:42px;line-height:1;margin-bottom:10px;">✅</div>
            <h3 style="margin:0 0 8px;font-weight:800;color:#0f172a;font-size:20px;">Your Shift Has Ended</h3>
            <p style="color:#555;font-size:14px;margin:0 0 20px;">
                You checked out at <strong>{{ $__att->check_out }}</strong>. The portal is locked until your
                next shift — please check in again to continue working.
            </p>
        @else
            <div style="font-size:42px;line-height:1;margin-bottom:10px;">🕒</div>
        ...
        @if($__hrBase)
            <a href="{{ route('hr.portal.redirect') }}?to=dashboard"
               style="display:inline-block;background:#1a73e8;color:#fff;text-decoration:none;border-radius:8px;padding:12px 30px;font-size:15px;font-weight:700;">
                {{ ($__shiftEnded ?? false) ? 'Go to HR Portal →' : 'Go to Check In →' }}
            </a>
        @endif
    </div>
</div>
