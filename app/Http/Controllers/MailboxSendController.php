...
class MailboxSendController extends Controller
{
    ...
    /**
     * The name the recipient sees in "From".
     *
     * Client report (19 Sep 2026): an agent sending from dave@hellotransport.com arrived as
     * "Muhammed Akbar" — his real name — instead of his Sudo Name "Dave". Two things were wrong:
     *
     *  1. It used the real name. Agents work under a Sudo Name (user.slug) precisely so customers
     *     do not see the real one, so the slug has to win whenever it is set.
     *  2. It used Auth::user() — whoever is logged in. With #7 send-as, a supervisor sending from
     *     an agent's mailbox put the SUPERVISOR's name on mail leaving the agent's address. The
     *     name must come from the mailbox's owner, matching the address it is sent from.
     *
     * Falls back the safe way: slug -> owner's real name -> the address' local part.
     */
    private function senderDisplayName(EmailAccount $acc): string
    {
        $owner = $acc->user;   // the agent this mailbox belongs to, not whoever is logged in

        $slug = trim((string) ($owner->slug ?? ''));
        $name = trim((string) ($owner->name ?? ''));

        if ($slug !== '' && ! $this->slugIsAutoGenerated($slug, $owner)) {
            return $slug;
        }
        ...
        $local = trim((string) ($acc->local_part ?? ''));

        return $local !== '' ? $local : 'Washington Agent';
    }

    /**
     * `slug` holds two different things, and only one of them is a Sudo Name.
     *
     * An admin typing "Dave" into Sudo Name on Edit Subcontractor is a real alias. But public
     * signup and the CrazyRays bridge also auto-fill slug when none was given, from the person's
     * own name — PublicSignupController makes "new.test", BridgeAuthController makes "zara-ali".
     * Putting those on outgoing customer mail would read worse than the real name.
     *
     * So a slug counts as auto-generated when it is just the owner's own name slugified, in any of
     * the forms those two places produce. Anything else was chosen by a human and is used as-is.
     */
    private function slugIsAutoGenerated(string $slug, $owner): bool
    {
        $slug  = Str::lower($slug);
        $first = trim((string) ($owner->name ?? ''));
        $last  = trim((string) ($owner->last_name ?? ''));
        $full  = trim($first . ' ' . $last);

        $auto = [];
        foreach (['-', '.'] as $sep) {
            foreach (array_filter([$full, $first]) as $basis) {
                $auto[] = Str::lower(Str::slug($basis, $sep));
            }
        }
        // EmployeeSyncController builds it from the address instead.
        if ($owner->email ?? null) {
            $auto[] = Str::lower(Str::slug(strtok((string) $owner->email, '@')));
        }

        // Signup appends a counter on collision ("new.test2"), so match that too.
        $bare = rtrim($slug, '0123456789');

        return in_array($slug, $auto, true) || ($bare !== '' && in_array($bare, $auto, true));
    }
    ...
    public function send(Request $request)
    {
        ...
        $fromName  = $this->senderDisplayName($acc);
